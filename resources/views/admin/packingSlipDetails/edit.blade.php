...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.packingSlipDetail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.packing-slip-details.update", [$packingSlipDetail->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="packing_slip_number">{{ trans('cruds.packingSlipDetail.fields.packing_slip_number') }}</label>
                <input class="form-control {{ $errors->has('packing_slip_number') ? 'is-invalid' : '' }}" type="number" name="packing_slip_number" id="packing_slip_number" value="{{ old('packing_slip_number', $packingSlipDetail->packing_slip_number) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('packing_slip_item_details') ? 'is-invalid' : '' }}" name="packing_slip_item_details[]" id="packing_slip_item_details" multiple>
                    @foreach($packing_slip_item_details as $id => $packing_slip_item_details)
                        <option value="{{ $id }}" {{ (in_array($id, old('packing_slip_item_details', [])) || $packingSlipDetail->packing_slip_item_details->contains($id)) ? 'selected' : '' }}>{{ $packing_slip_item_details }}</option>
                    @endforeach
                </select>
