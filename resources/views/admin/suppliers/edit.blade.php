    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.suppliers.update", [$supplier->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('address') ? 'is-invalid' : '' }}" name="address" id="address" required>{{ old('address', $supplier->address) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pending_invoices') ? 'is-invalid' : '' }}" name="pending_invoices[]" id="pending_invoices" multiple required>
                    @foreach($pending_invoices as $id => $pending_invoices)
                        <option value="{{ $id }}" {{ (in_array($id, old('pending_invoices', [])) || $supplier->pending_invoices->contains($id)) ? 'selected' : '' }}>{{ $pending_invoices }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('purchase_orders') ? 'is-invalid' : '' }}" name="purchase_orders[]" id="purchase_orders" multiple required>
                    @foreach($purchase_orders as $id => $purchase_orders)
                        <option value="{{ $id }}" {{ (in_array($id, old('purchase_orders', [])) || $supplier->purchase_orders->contains($id)) ? 'selected' : '' }}>{{ $purchase_orders }}</option>
                    @endforeach
                </select>
