            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('purchased_items') ? 'is-invalid' : '' }}" name="purchased_items[]" id="purchased_items" multiple required>
                    @foreach($purchased_items as $id => $purchased_items)
                        <option value="{{ $id }}" {{ (in_array($id, old('purchased_items', [])) || $crmDocument->purchased_items->contains($id)) ? 'selected' : '' }}>{{ $purchased_items }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pending_invoice') ? 'is-invalid' : '' }}" name="pending_invoice_id" id="pending_invoice_id">
                    @foreach($pending_invoices as $id => $pending_invoice)
                        <option value="{{ $id }}" {{ (old('pending_invoice_id') ? old('pending_invoice_id') : $crmDocument->pending_invoice->id ?? '') == $id ? 'selected' : '' }}>{{ $pending_invoice }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('packing_slip_details') ? 'is-invalid' : '' }}" name="packing_slip_details[]" id="packing_slip_details" multiple>
                    @foreach($packing_slip_details as $id => $packing_slip_detail)
                        <option value="{{ $id }}" {{ (in_array($id, old('packing_slip_details', [])) || $crmDocument->packing_slip_details->contains($id)) ? 'selected' : '' }}>{{ $packing_slip_detail }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status_id" id="status_id">
                    @foreach($statuses as $id => $status)
                        <option value="{{ $id }}" {{ (old('status_id') ? old('status_id') : $crmDocument->status->id ?? '') == $id ? 'selected' : '' }}>{{ $status }}</option>
                    @endforeach
                </select>
