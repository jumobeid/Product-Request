...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.update", [$product->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="packing_slip_item_details">{{ trans('cruds.product.fields.packing_slip_item_details') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('packing_slip_item_details') ? 'is-invalid' : '' }}" name="packing_slip_item_details[]" id="packing_slip_item_details" multiple>
                    @foreach($packing_slip_item_details as $id => $packing_slip_item_details)
                        <option value="{{ $id }}" {{ (in_array($id, old('packing_slip_item_details', [])) || $product->packing_slip_item_details->contains($id)) ? 'selected' : '' }}>{{ $packing_slip_item_details }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.product.fields.packing_slip_item_details_helper') }}</span>
            </div>
            <div class="form-group">
